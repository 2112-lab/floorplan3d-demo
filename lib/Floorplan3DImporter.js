...
import { convertSVG, parseSvgDimension, parseSvgToPath } from "./svg";
import { getPointsFromPaths, svgToKonvaObjects } from "./konva/konva";
...
class Floorplan3DImporter {
  ...
  importSvg(svgStr) {
    ...
    try {
      ...
      const objects = paths.length > 0 ? getPointsFromPaths(paths) : {};
      ...
      const document = {
        ...
        svgPath: svgStr,
        svgPolyline: convertSVG(svgStr, "pathToPolyline"),
        svgData: {
          element: svgElement,
          svgString: svgStr
        },
    ...
    try {
      ...
      for (const element of floorplan3dElements) {
        ...
        if (isRaster) {
          ...
          const svgPolyline = convertSVG(layerSvg, "pathToPolyline");
