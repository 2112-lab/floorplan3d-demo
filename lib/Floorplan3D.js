...
export default class Floorplan3D {
  ...
  async renderSvgToScene(svg, documentId = null) {
    ...
    if (documentId && useSvgStore().documents[documentId]) {
      height =
        useSvgStore().documents[documentId].docConfigs.extrusion.height
          .value || 10;
    ...
    // Get current settings from the svg store
    const currentOpacity =
      documentId && useSvgStore().documents[documentId]
        ? useSvgStore().documents[documentId].docConfigs.extrusion.opacity
            .value || 0.8
        : 0.8;
    ...
    if (createdObjects.length > 0) {
      ...
      createdObjects.forEach((obj) => {      
        ...
        const verticalPosition = (
          documentId && 
          useSvgStore().documents[documentId] && 
          useSvgStore().documents[documentId].docConfigs.extrusion.verticalPosition?.value
        ) ? useSvgStore().documents[documentId].docConfigs.extrusion.verticalPosition?.value : 0;
    ...
    if (documentId && useSvgStore().documents[documentId]) {
      ...
      useSvgStore().documents[documentId].threejsContent = {
        objects: createdObjects,
        documentId: documentId,
      };
    } else if (documentId) {
      console.warn(`Document with ID ${documentId} not found in SVG store`);
    }
    ...
    console.log(
      ...
      JSON.parse(JSON.stringify(useSvgStore().documents[documentId]))
    );
