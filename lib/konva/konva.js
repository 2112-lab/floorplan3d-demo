...
import { convertColorToHex, getRandomId } from "./utils";
import { addHandlesToPolyline, handleHandleDoubleClick, handlePolylineClick, handlePolylineDoubleClick, handlePolylineHandleClick, polyLineHandleDragEnd, polyLineHandleDragMove, polyLineHandleDragStart } from "./polyline-events";
...
// UNIFIED SVG EDITING FUNCTIONS
export function svgToKonvaObjects(svgString, doc_id) {
  const svgElement = parseSvgToPath(svgString);
  const paths = svgElement.querySelectorAll("path");
  let objects = {};
  // Handle paths
  if (paths.length > 0) {
    objects = { ...objects, ...getPointsFromPaths(paths) };
  }

  const konvaStore = useKonvaStore();
  konvaStore.setKonvaObjects(doc_id, objects);
  return { objects };
}
...
export function renderSvgInKonva(layer, objects) {
  ...
  Object.values(objects).forEach((object, idx) => {
    ...
    polyLine.on("dblclick", handlePolylineDoubleClick);

    polyLineGroup.add(polyLine);
    layer.add(polyLineGroup);

    // If the polyline is selected, create and add handles
    if (object.selected) {
      const handles = addHandlesToPolyline(object, idx);
